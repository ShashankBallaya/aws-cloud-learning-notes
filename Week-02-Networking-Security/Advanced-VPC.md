...
## What Is It?
Advanced VPC setup lets you run provate servers securely while still allowing controlled internet access and monitoring network traffic.
...
- Bastion Host: Public EC2 used as a secure entry point to provate instances.
- Security Group Referencing: Allows traffic only from another SG instead of IP.
- Route Table (Private): Routes internet-bound traffic only from another SG instead of IP.
- No Public IP: Ensures instance is not directly reachable from internet.
...
- Launched app server EC2 in private subnet without public IP.
...
- Tested outbound internet from private EC2:
	- Ran `curl https://checkip.amazonaws.com` - recieved response.
	- Ran `sudo yum update -y` - packages downloaded successfuly.
- Enables VPC Flow Logs on `week1-vpc` and sent logs to CloudWatch.
- Verified ACCEPT entries for SSH traffic in flow logs.
- Deleted NAT Gateway and released Elastic IP after testing to avoid costs.
...
sudo yum update -y
# Test outbounf connectivity by installing updates
```

## What Broke / What Confused ME
Nothing broke - but I want to remember: private subnet instances cannot access the internet unless the route table is correctly configure to use a NAT Gateway.

## One_Line Summary 
Advanced VP setups use NM Gateway, bastion host, and private subnets to securely isolate systems while maintaining controlled inetnet access.
